...
namespace tiny_rl
{
    class PPOTrainer : public BaseTrainer
    {
    public:
        PPOTrainer(PPOAgent &agent, std::shared_ptr<BaseEnv> env)
            : BaseTrainer(agent, env),
              agent_(agent),
              paused_(false)
        {
        }
        ...
        void train(int episodes) override
        {
            ...
            std::thread input_thread([this, &stop_input_thread]()
                ...
                while (!stop_input_thread)
                {
                    ...
                    if (input == "pause")
                    {
                        std::cout << "Training will pause after current episode..." << std::endl;
                        paused_ = true;
                    }
